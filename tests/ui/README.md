...
该测试把函数从 `ui.html` 切片出来直接执行（不复制源码，因此不会与线上版本漂移），逐一断言：无观测、长期无流量、注入后仍受限（报警）、新鲜自然观测、注入盲区、陈旧读数，以及标签必须带样本数 `n=`。

已验证它能拦住回归：把 `OBS_STALE_MS` 调大到盲区不再被识别，盲区那格会显示成 `● 正常 (292)`，测试转红。
